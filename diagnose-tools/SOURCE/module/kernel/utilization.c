...
int utilization_syscall(struct pt_regs *regs, long id)
{
	...
	switch (id) {
	case DIAG_UTILIZATION_SET:
		user_buf = (void __user *)SYSCALL_PARAM1(regs);
		user_buf_len = (size_t)SYSCALL_PARAM2(regs);

		if (user_buf_len != sizeof(struct diag_utilization_settings)) {
			ret = -EINVAL;
		} else if (utilization_settings.activated) {
			ret = -EBUSY;
		} else {
		...
		break;
	case DIAG_UTILIZATION_SETTINGS:
		user_buf = (void __user *)SYSCALL_PARAM1(regs);
		user_buf_len = (size_t)SYSCALL_PARAM2(regs);
	...
	case DIAG_UTILIZATION_ISOLATE:
		cpu = SYSCALL_PARAM1(regs);
		user_buf = (void __user *)SYSCALL_PARAM2(regs);
		user_buf_len = (size_t)SYSCALL_PARAM3(regs);
		if (user_buf_len >= CGROUP_NAME_LEN)
			user_buf_len = CGROUP_NAME_LEN - 1;
