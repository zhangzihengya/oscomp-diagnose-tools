...
static int drop_packet_alloced = 0;
...
struct conn_info
{
	...
	atomic64_t sum_truesize[TRACK_COUNT];
	atomic64_t sum_len[TRACK_COUNT];
	atomic64_t sum_datalen[TRACK_COUNT];
...
static struct diag_variant_buffer drop_packet_variant_buffer;
	...
	unsigned int dport)
{
	...
	if (direct == IN && !drop_packet_settings.arrange_by_peer)
...
static noinline void inspect_packet(const struct sk_buff *skb, const struct iphdr *iphdr, enum packet_step step)
{
	...
	conn_info = find_alloc_desc(step <= RCV_STEP ? IN : OUT,
	...
	atomic64_add(skb->len, &conn_info->sum_len[step]);
	atomic64_add(skb->data_len, &conn_info->sum_datalen[step]);
}
...
static int diag_ip_local_deliver(struct sk_buff *skb)
{
	...
	iph = ip_hdr(skb);
	inspect_packet(skb, iph, LOCAL_DELIVER);
	...
	inspect_packet(skb, iph, LOCAL_DELIVER_FINISH);
...
static int diag_ip_rcv_finish(struct net *net, struct sock *sk, struct sk_buff *skb)
{
	...
	inspect_packet(skb, iph, DST_INPUT);
...
static int kprobe_eth_type_trans_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	inspect_packet(skb, iphdr, ETH_RECV);
...
static int kprobe_napi_gro_receive_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	inspect_packet(skb, iphdr, GRO_RECV);
...
static int diag_napi_gro_complete(struct sk_buff *skb)
{
	...
	if (err) {
		...
		iphdr = (struct iphdr *)skb->data;
		inspect_packet(skb, iphdr, GRO_RECV_ERR);
...
static int diag_netif_receive_skb(struct sk_buff *skb)
{
	...
	if (ret == NET_RX_DROP) {
		...
		iphdr = (struct iphdr *)skb->data;
		inspect_packet(skb, iphdr, RECV_SKB_DROP);
	}
...
static int kprobe___netif_receive_skb_core_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	iphdr = (struct iphdr *)skb->data;
	inspect_packet(skb, iphdr, RECV_SKB);
...
__maybe_unused static int kprobe_ip_rcv_finish_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	iphdr = ip_hdr(skb);
	inspect_packet(skb, iphdr, IP_RCV_FINISH);
...
static int kprobe_udp_rcv_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	iphdr = ip_hdr(skb);
	inspect_packet(skb, iphdr, UDP_RCV);
...
static int kprobe_tcp_v4_rcv_pre(struct kprobe *p, struct pt_regs *regs)
{
	...
	iphdr = ip_hdr(skb);
	inspect_packet(skb, iphdr, TCP_V4_RCV);
...
int __activate_drop_packet(void)
{
	int ret = 1;

	ret = alloc_diag_variant_buffer(&drop_packet_variant_buffer);
	if (ret)
		goto out_variant_buffer;
	drop_packet_alloced = 1;
...
{
	if (!drop_packet_settings.activated)
		drop_packet_settings.activated = __activate_drop_packet();

	return drop_packet_settings.activated;
}

int deactivate_drop_packet(void)
{
	if (drop_packet_settings.activated)
		__deactivate_drop_packet();
	drop_packet_settings.activated = 0;
...
static void do_dump(void)
{
	...
	{
		detail.et_type = et_drop_packet_detail;
		...
		for (i = 0; i < TRACK_COUNT; i++) {
			...
			detail.sum_len[i] = atomic64_read(&this->sum_len[i]);
			detail.sum_datalen[i] = atomic64_read(&this->sum_datalen[i]);
		}
		diag_variant_buffer_spin_lock(&drop_packet_variant_buffer, flags);
		diag_variant_buffer_reserve(&drop_packet_variant_buffer, sizeof(struct drop_packet_detail));
		diag_variant_buffer_write_nolock(&drop_packet_variant_buffer, &detail, sizeof(struct drop_packet_detail));
		diag_variant_buffer_seal(&drop_packet_variant_buffer);
		diag_variant_buffer_spin_unlock(&drop_packet_variant_buffer, flags);
	}
...
int drop_packet_syscall(struct pt_regs *regs, long id)
{
	...
	switch (id) {
	case DIAG_DROP_PACKET_SET:
		user_buf = (void __user *)SYSCALL_PARAM1(regs);
		user_buf_len = (size_t)SYSCALL_PARAM2(regs);

		if (user_buf_len != sizeof(struct diag_drop_packet_settings)) {
			ret = -EINVAL;
		} else if (drop_packet_settings.activated) {
			ret = -EBUSY;
		} else {
		...
		break;
	case DIAG_DROP_PACKET_SETTINGS:
		user_buf = (void __user *)SYSCALL_PARAM1(regs);
		user_buf_len = (size_t)SYSCALL_PARAM2(regs);
...
int diag_net_drop_packet_init(void)
{
	...
	init_diag_variant_buffer(&drop_packet_variant_buffer, 20 * 1024 * 1024);
	jump_init();
...
void diag_net_drop_packet_exit(void)
{
	...
	drop_packet_settings.activated = 0;
	destroy_diag_variant_buffer(&drop_packet_variant_buffer);
